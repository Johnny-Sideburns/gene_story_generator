...
(:predicates ;todo: define predicates here
    ...
    (ambushing ?loc - location ?mon - monster)
)
...
(:action move
    ...
    (forall (?mon - monster) (not (ambushing ?mon ?from)))
    )
...
(:action go_to_sleep
    ...
    :precondition (and (whereabouts ?loc ?char) (not (isasleep ?char)) (not (isdead ?char)) (not (ambushing ?char ?loc))
...
(:action push_into_hazzard
    ...
    :effect (and (isdead ?mon) (not (ambushing ?mon ?loc))
    (increase (total-cost) 1)
    )
)
(:action set_ambush
    :parameters (?mon - monster ?loc - location)
    :precondition (and (whereabouts ?loc ?mon) (not (isdead ?mon)) (not (isasleep ?mon)))
    :effect (and (ambushing ?loc ?mon)
    (increase (total-cost) 1)
    )
)
)